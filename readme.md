...
In this way our "AnalyticsProducer" is able to retrieve the necessary data in an efficient way. 
...
Our Architecture is composed of abstract classes and methods that explicit the interactions betweeen the OneStream component, the Historical DB and the external applications.
These classes are the technical basis for the design process that will follow.
...
AnalyticsProducer: Class used to produce analytics from data retrieved from the Historical DB.
The produced analytics will be available through our API.

analytics_api: Public API that exposes the get_analytics method to external applications and provides the data analytics. These application will be in charge of drawing the analytics in charts, ecc.
